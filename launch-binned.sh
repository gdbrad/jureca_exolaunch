...
usage() {
    ...
    echo "  -e <ensemble>   : Specify the ensemble (e.g., 'eric-L32T64', 'gio-L32T96')"
...
# Validate type
if [ "$type" != "meson" ] && [ "$type" != "peram" ] && [ "$type" != "peram_strange" ] && [ "$type" != "peram_charm" ] && [ "$type" != "eigs" ]; then
...
#     BASE_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble/"
# else
BASE_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble"
#fi
...
#source $myenv
#source $formenv
#CHROMA_BIN=$CODE_DIR/install/chroma-quda-qdp-jit-double-nd4-cmake-superbblas-cuda/bin
...
if [ "$convert" = true ]; then
    ...
    if [ "$type" = "peram" ]; then
        SDB_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble/perams_sdb/numvec$numvec"
    elif [ "$type" = "peram_strange" ]; then
        SDB_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble/perams_strange_sdb"
    elif [ "$type" = "peram_charm" ]; then
         SDB_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble/perams_charm_sdb/numvec$numvec"

    elif [ "$type" = "eigs" ]; then
        SDB_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble/eigs_sdb"
    else
#        SDB_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble/${type}_sdb/numvec$numvec"
        SDB_DIR="/p/scratch/exotichadrons/exolaunch/$ensemble/${type}_sdb/numvec$numvec"
            ...
            if [[ "${cfg_numbers[*]}" =~ (^|[[:space:]])$cfg_num($|[[:space:]]) ]] || [ ${#cfg_numbers[@]} -eq 0 ]; then
                ...
                if [ ! -f "$h5_file" ] || ([ "$type" = "meson" ] && [ "$numvec" -eq 32 ] && ! check_file_size "$h5_file" "$min_size"); then
                    ...
                    if [ "$test" = true ]; then
                        ...
                        srun --account=exotichadrons --partition=dc-gpu -t 02:00:00 --nodes=1 --ntasks-per-node=1 --gres=gpu:1 --threads-per-core=1 -n 1 -c 1 $object $OPTS -i $in -o $out -l $log > $stdout 2>&1
                    fi
                fi
            fi
        else
